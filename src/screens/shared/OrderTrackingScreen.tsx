...
import { View, Text, StyleSheet, Dimensions, TouchableOpacity, ActivityIndicator, Animated, PanResponder } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Ionicons } from '@expo/vector-icons';
import MapView, { Marker, Polyline } from 'react-native-maps';
import LottieView from 'lottie-react-native';
import { getOrderAddress, subscribeToOrderUpdates } from '../../services/order.service';
...
export default function OrderTrackingScreen({ navigation, route }: any) {
  ...
  const [deliveryStartedAt, setDeliveryStartedAt] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToOrderUpdates(orderId, (newOrderData) => {
      ...
      if (newOrderData.delivery_started_at) setDeliveryStartedAt(newOrderData.delivery_started_at);
    });
  ...
  useEffect(() => {
    if (!loading && customerLocation && mapRef.current) {
      mapRef.current.fitToCoordinates([STORE_LOCATION, shipperLocation, customerLocation], {
      ...
      {loading ? (
        ...
        >
          ...
          <Marker coordinate={STORE_LOCATION} title="Nhà hàng">
            <View style={[styles.markerContainer, { backgroundColor: '#181C2E' }]}>
          ...
          {customerLocation && (
            <Marker coordinate={customerLocation} title="Bạn">
              <View style={[styles.markerContainer, { backgroundColor: '#FF7622' }]}>
      ...
      {orderStatus === 'delivering' && (
        ...
        >
          ...
          <View style={styles.sheetContent}>
            <Text style={styles.estimatedText}>THỜI GIAN GIAO HÀNG DỰ KIẾN</Text>
